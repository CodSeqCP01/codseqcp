...
Help_text= "Usage: python run_augustus.py species e.g python run_augustus.py Danio_rerio"
...
def augustus(scaffolds, outfilename, species=None):
    ...
    augustus_cmd_list.append("{} {}.gff".format(">", outfilename))
...
def main():
    ...
    Dirs = [SPAdir for SPAdir in os.listdir(os.getcwd()) if os.path.isdir(os.path.join(os.getcwd(), SPAdir))]
    for Dir in Dirs:
        rootdir = os.path.join(os.getcwd(), Dir)
        for file in os.listdir(rootdir):
            if os.path.isfile(os.path.join(rootdir, file)) and file.endswith("fasta") and "scaffolds" in file:
                if sps:
                    sps = sys.argv[1]
                    augustus(os.path.join(rootdir, file), rootdir.split("/")[-1], species=sps)
                else:
